...
class SimpleTextRemover:
    def __init__(self, 
        ...
        """
        self.detector = SimpleTextDetector()
    ...
    def _create_expanded_mask(self, image, boxes):
        """
    ...
    def remove_text(self, image):
        ...
        # 创建扩展遮罩
        expanded_mask = self._create_expanded_mask(image, boxes)
...
def test_text_removal():
    ...
    try:
        ...
        # 保存去除文本后的图像
        result_path = os.path.join(output_dir, f"{base_name}_removed{ext}")
        ...
        # 保存精细遮罩
        refined_mask_path = os.path.join(output_dir, f"{base_name}_refined_mask{ext}")
