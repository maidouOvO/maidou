...
class CRAFTTextRemover:
    ...
    def __init__(self, 
                 ...
                 cuda=False,
                 padding=7,           # 增加填充大小以确保覆盖整个文本区域
                 inpaint_method=cv2.INPAINT_TELEA,
                 min_area=50,         # 降低最小面积以检测更多文本区域
                 min_aspect_ratio=0.1, # 放宽纵横比限制
                 max_aspect_ratio=15,  # 放宽纵横比限制
                 epsilon_factor=0.02,
                 char_min_area=20,
                 char_max_area=10000,  # 大幅增大最大字符面积以处理大字体
                 char_min_aspect_ratio=0.1, # 放宽字符纵横比限制
                 char_max_aspect_ratio=15,  # 放宽字符纵横比限制
                 char_min_solidity=0.2,     # 降低最小实心度以检测更多文本
                 dilation_kernel_size=7):   # 增大膨胀核大小以确保覆盖整个文本
        """
        初始化CRAFT文本去除器，专门针对大字体文本优化
    ...
    def _detect_text_regions(self, image):
        ...
        # 2. 连接相近的文本区域
        connect_kernel = np.ones((25, 5), np.uint8)  # 水平连接
        region_mask = cv2.morphologyEx(region_mask, cv2.MORPH_CLOSE, connect_kernel)
        
        connect_kernel = np.ones((5, 25), np.uint8)  # 垂直连接
        region_mask = cv2.morphologyEx(region_mask, cv2.MORPH_CLOSE, connect_kernel)
    ...
    def _detect_text_shapes(self, image, region_mask):
        ...
        # 2. 自适应阈值 - 小块大小，适用于小字体文本
        binary_adaptive1 = cv2.adaptiveThreshold(
            gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
            cv2.THRESH_BINARY_INV, 15, 5
        )
        binary_adaptive1 = cv2.bitwise_and(binary_adaptive1, region_mask)
        
        # 3. 自适应阈值 - 大块大小，适用于大字体文本
        binary_adaptive2 = cv2.adaptiveThreshold(
            gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
            cv2.THRESH_BINARY_INV, 35, 10
        )
        binary_adaptive2 = cv2.bitwise_and(binary_adaptive2, region_mask)
        
        # 4. 固定阈值（针对大字体）
        _, binary_fixed1 = cv2.threshold(gray, 180, 255, cv2.THRESH_BINARY_INV)
        binary_fixed1 = cv2.bitwise_and(binary_fixed1, region_mask)
        
        # 5. 固定阈值（针对中等对比度文本）
        _, binary_fixed2 = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY_INV)
        binary_fixed2 = cv2.bitwise_and(binary_fixed2, region_mask)
        
        # 合并二值化结果
        binary_combined = cv2.bitwise_or(binary_global, binary_adaptive1)
        binary_combined = cv2.bitwise_or(binary_combined, binary_adaptive2)
        binary_combined = cv2.bitwise_or(binary_combined, binary_fixed1)
        binary_combined = cv2.bitwise_or(binary_combined, binary_fixed2)
    ...
    def _filter_character_features(self, mask, image):
        ...
        for contour in contours:
            ...
            # 检查是否为文本特征 - 放宽条件以适应大字体文本
            is_text_feature = (
                # 文本通常具有适中的纵横比，但大字体可能更宽
                0.1 < aspect_ratio < 15 and
                # 文本通常具有较高的复杂度
                complexity > 0.05 and
                # 文本通常具有适中的实心度，但大字体可能更低
                0.2 < solidity < 0.95 and
                # 过滤掉过小的轮廓，但允许更大的轮廓通过
                20 < area < 10000
            )
            ...
            # 对于大字体文本，我们可以放宽孤立特征的要求
            if is_text_feature and (is_isolated or area > 1000):
                # 如果是文本特征，则添加到过滤后的遮罩
    ...
    def _is_isolated_feature(self, contour, gray_image):
        ...
        # 高对比度表示孤立特征，但对于大字体文本，我们可以降低阈值
        return contrast > 20
        
    def remove_text(self, image):
        """
        去除图像中的文本，特别优化用于去除大字体文本
        ...
        # 使用图像修复去除文本
        result = cv2.inpaint(image, filtered_mask, 5, self.inpaint_method)
        
        return result, region_mask, filtered_mask

def test_craft_text_removal():
    """
    测试CRAFT文本去除功能，特别针对大字体文本
    """
    # 设置参数
    ...
    try:
        ...
        # 保存原始图像
        original_path = os.path.join(output_dir, f"{base_name}_craft_original{ext}")
        cv2.imwrite(original_path, image)
        
        # 保存去除文本后的图像
        result_path = os.path.join(output_dir, f"{base_name}_craft_result{ext}")
        cv2.imwrite(result_path, result)
        
        # 保存文本区域遮罩
        region_mask_path = os.path.join(output_dir, f"{base_name}_craft_region_mask{ext}")
        cv2.imwrite(region_mask_path, region_mask)
        
        # 保存文本形状遮罩
        filtered_mask_path = os.path.join(output_dir, f"{base_name}_craft_filtered_mask{ext}")
